...
execute at @e[tag=craft1] as @e[tag=craft2,distance=..1] as @e[tag=craft3,distance=..1] as @e[tag=craft4,distance=..1] as @e[tag=craft5,distance=..1] as @e[tag=craft6,distance=..1] as @e[tag=craft7,distance=..1] as @e[tag=craft8,distance=..1] as @e[tag=craft9,distance=..1] run summon item ~ ~ ~ {Tags:["itemkill1"],PickupDelay:20,Item:{id:"minecraft:netherite_axe",Count:1b,tag:{display:{Name:'{"text":"Lumberjack axe","color":"dark_purple","italic":true}',Lore:['{"text":"Chops down a whole tree at once!"}']},Enchantments:[{}]}}}
